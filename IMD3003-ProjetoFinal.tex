...
A projeção UMAP 3D foi obtida por meio de um \textit{grid search} refinado, no qual foram avaliadas combinações dos parâmetros \textit{n\_neighbors} iguais a 10, 15, 20 e 25, \textit{min\_dist} iguais a 0.0, 0.05, 0.10 e 0.20, utilizando a métrica de similaridade cosseno. Para cada configuração, a qualidade da projeção tridimensional foi avaliada por meio da métrica de \textit{trustworthiness}, calculada com \texttt{n\_neighbors = 10}, comparando as relações de vizinhança no espaço original de 384 dimensões com aquelas preservadas no espaço reduzido.

A melhor configuração identificada apresentou valores elevados de trustworthiness, indicando preservação consistente das estruturas locais após a redução dimensional. Essa projeção foi adotada como base para as etapas subsequentes de clusterização no espaço tridimensional.

Inicialmente, foi aplicado o algoritmo DBSCAN sobre a projeção UMAP 3D selecionada, com o objetivo de identificar agrupamentos densos e regiões de ruído. Embora essa abordagem tenha permitido separar estruturas globais, observou-se novamente a formação de um agrupamento dominante concentrando uma parcela significativa das instâncias, caracterizando a presença de um supercluster.
\subsection{UMAP 3D}
